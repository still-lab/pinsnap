...
- [ ] C-040 检测屏幕录制权限 API 封装
- [ ] C-041 无权限 → 展示引导窗（UI_SPEC §2）
- [ ] C-042 「打开系统设置」深链
- [ ] C-043 「稍后」关闭
- [ ] C-044 从设置返回后权限变化监听/下次热键重试
- [ ] C-045 文案本地化中/英键
...
- [ ] F-001 关闭 → closedStack
- [ ] F-002 恢复栈容量配置（默认 1 或 5，写死并文档化）
- [ ] F-003 贴图热键恢复关闭项逻辑
- [ ] F-004 销毁删文件+模型
- [ ] F-005 隐藏全部 / 显示全部
...
- [ ] F-025 PinGroup 模型 CRUD
- [ ] F-026 切换活动组（显隐）
- [ ] F-027 组管理简易 UI（列表）
- [ ] F-028 FeatureGate.pinGroups
...
- [ ] F-040 纯文本 → TextKit 渲染图
- [ ] F-041 HTML 简化或当纯文本
- [ ] F-042 颜色 `#RGB` / `rgb()` → 色卡
- [ ] F-043 文件 URL 图片加载（书签/权限）
